...
inline void cancel_reservation(memory::region_t reserved) {
    ...
    throw_if_error(status, "Failed feeing a reservation of memory region at "
                           + cuda::detail_::ptr_as_hex(reserved.data()) + " of size " + ::std::to_string(reserved.size()));
...
inline reservation_t reserve(region_t region, alignment_t alignment)
{
    ...
    throw_if_error(status,
        ...
        + cuda::detail_::ptr_as_hex(region.data()) + " of size " + ::std::to_string(region.size()));
...
using shared_handle_t = typename detail_::shared_handle_type_helper<Kind>::type;
...
public: // non-mutators
    friend allocation_t allocation::detail_::wrap(allocation::handle_t handle, size_t size, bool owning);
...
allocation_t import(shared_handle_t<SharedHandleKind> shared_handle, size_t size, bool take_ownership = false)
{
    ...
    return allocation::detail_::wrap(result_handle, size, take_ownership);
}
...
public: // constructors & destructions
    ...
    friend mapping_t mapping::detail_::wrap(region_t region, bool owning);
    ...
    ~mapping_t()
    {
        ...
        throw_if_error(result,
        	...
        	+ cuda::detail_::ptr_as_hex(region_.data()) + " of size " + ::std::to_string(region_.size()) + " bytes");
    }
...
public:
	...
	allocation_t allocation() const
    {
        ...
        return allocation::detail_::wrap(allocation_handle, region_.size(), dont_take_ownership);
    }
#endif
protected:
